...
.vt_circles_time <- function(d, y_col, vt1_time, vt2_time,
  ...
  layers <- list()
  if (is.finite(vt1_time) && y_col %in% names(d)) {
    idx <- which.min(abs(d$time_min - vt1_time))
    yv  <- d[[y_col]][idx]
    if (length(yv) == 1 && is.finite(yv))
      layers <- c(layers, list(
        ggplot2::annotate("point", x = vt1_time, y = yv,
                          shape = 21, size = 2.5, stroke = 1.2,
                          colour = col_vt1, fill = NA)
      ))
  }
  if (is.finite(vt2_time) && y_col %in% names(d)) {
    idx <- which.min(abs(d$time_min - vt2_time))
    yv  <- d[[y_col]][idx]
    if (length(yv) == 1 && is.finite(yv))
      layers <- c(layers, list(
        ggplot2::annotate("point", x = vt2_time, y = yv,
                          shape = 21, size = 2.5, stroke = 1.2,
                          colour = col_vt2, fill = NA)
      ))
  }
  layers
}
...
.vt_circles_xy <- function(d, x_col, y_col, vt, pattern,
  ...
  idx <- which.min(abs(d[[x_col]] - vx))
  if (length(idx) == 1 && is.finite(d[[x_col]][idx]) && is.finite(d[[y_col]][idx]))
    layers <- c(layers, list(
      ggplot2::annotate("point", x = d[[x_col]][idx], y = d[[y_col]][idx],
                        shape = 21, size = 2.5, stroke = 1.2,
                        colour = col, fill = NA)
    ))
  layers
