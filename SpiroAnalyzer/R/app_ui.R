...
app_ui <- function(request) {
  ...
  shiny::navbarPage(
    ...
    shiny::tabPanel(
      ...
      value = "start",
      shiny::fluidPage(
        shiny::div(class = "hero-outer",
          shiny::div(class = "hero-eyebrow", "Spiroergometrie \u2022 Analyse \u2022 Vergleich"),
          shiny::div(class = "hero-title", "SpiroAnalyzer"),
          shiny::div(class = "hero-sub",
            "Lade XML- oder Excel-Dateien der MetaLyzer-Spiroergometrie, ",
            "visualisiere Einzelmessungen und vergleiche T0 vs. T10 auf einen Blick."
          ),
          shiny::div(class = "hero-chips",
            shiny::div(class = "hero-chip", shiny::icon("file-code"), " XML & Excel"),
            shiny::div(class = "hero-chip", shiny::icon("chart-area"), " Dual-Achsen-Plot"),
            shiny::div(class = "hero-chip", shiny::icon("arrows-left-right"), " T0 / T10 Vergleich"),
            shiny::div(class = "hero-chip", shiny::icon("download"), " 600 dpi Export")
          )
        ),
        shiny::br(),
        shiny::tags$h5("So funktioniert es",
                       style = "font-weight:800; color:#1f3d6b; margin-bottom:14px;"),
        shiny::div(class = "steps-grid",
          shiny::div(class = "step-card",
            shiny::div(class = "step-badge", "1"),
            shiny::tags$h5("Datei laden"),
            shiny::tags$p("XML oder Excel-Datei aus der MetaLyzer-Software hochladen.")
          ),
          shiny::div(class = "step-card",
            shiny::div(class = "step-badge", "2"),
            shiny::tags$h5("Einzelanalyse"),
            shiny::tags$p("VO\u2082/kg-Linie, Power-Ribbon, PPO & Peak-Annotationen ansehen.")
          ),
          shiny::div(class = "step-card",
            shiny::div(class = "step-badge", "3"),
            shiny::tags$h5("T0 / T10 Vergleich"),
            shiny::tags$p("Zwei Messungen \u00fcbereinanderlegen & \u0394-Tabelle auswerten.")
          ),
          shiny::div(class = "step-card",
            shiny::div(class = "step-badge", "4"),
            shiny::tags$h5("Exportieren"),
            shiny::tags$p("Plot als 600 dpi PNG und Daten als CSV herunterladen.")
          )
        )
      )
    ),
    ...
    # ════════════════════════════════════════════════════════
    shiny::tabPanel(
      title = shiny::tagList(shiny::icon("magnifying-glass-chart"), " Einzelanalyse"),
      value = "single",
        ...
        shiny::div(class = "info-grid",
          ...
          shiny::div(class = "info-card",
            ...
            shiny::div(class = "info-cite-box",
              ...
              "[R Shiny App]. Institut f\u00fcr Kreislaufforschung und Sportmedizin, DSHS K\u00f6ln."
            ),
            shiny::tags$br(),
            shiny::div(class = "info-row",
